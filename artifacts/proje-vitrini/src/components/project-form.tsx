...
import { ArrowLeft, ArrowRight, Save, Send, X, Bot, ShieldCheck, Link as LinkIcon, Image as ImageIcon, LayoutList } from 'lucide-react';
...
interface ProjectFormProps {
  ...
  onSubmit: (status: 'draft' | 'pending', data: ProjectFormValues) => void;
  title: string;
  backHref: string;
  submitLabels: { draft: string; publish: string };
}

export function ProjectForm({ defaultValues, isSubmitting, onSubmit, title, backHref, submitLabels }: ProjectFormProps) {
  const [step, setStep] = useState(1);

  const form = useForm<ProjectFormValues>({
    resolver: zodResolver(projectFormSchema),
    defaultValues: { ...projectFormDefaultValues, ...defaultValues },
    mode: 'onChange',
  });
  ...
  const handleSubmitStatus = (status: 'draft' | 'pending') => async (e: React.MouseEvent) => {
    e.preventDefault();
    ...
    onSubmit(status, form.getValues());
  };
            ...
            <CardContent className="pt-6">
              ...
              <div className={step === 2 ? 'space-y-6 block' : 'hidden'}>
                ...
                <div className="space-y-4 pt-4 border-t">
                  <Label>Ekran Görüntüleri (İsteğe Bağlı)</Label>
                ...
                <div className="bg-[#3b82f6]/10 border border-[#3b82f6]/20 rounded-xl p-5">
                  ...
                  {watchAiUsed && (
                    <div className="mt-6 space-y-5 animate-in fade-in slide-in-from-top-4">
                      <div>
                        <Label className="text-sm font-semibold mb-2 block">Kullanılan Araçlar (Cursor, ChatGPT, Claude vb.)</Label>
                        <div className="flex gap-2">
                      ...
                      <div>
                        <Label className="text-sm font-semibold mb-2 block">Hangi Alanlarda Kullandınız?</Label>
                        <div className="flex gap-2">
            ...
            <div className="p-6 pt-4 border-t bg-muted/20 flex justify-between rounded-b-xl">
              ...
              {step < 4 ? (
                ...
                <div className="flex gap-3">
                  <Button type="button" variant="secondary" onClick={handleSubmitStatus('draft')} disabled={isSubmitting} className="gap-2">
                    <Save className="w-4 h-4" /> {submitLabels.draft}
                  </Button>
                  <Button type="button" onClick={handleSubmitStatus('pending')} disabled={isSubmitting} className="gap-2 font-bold shadow-md">
                    <Send className="w-4 h-4" /> {submitLabels.publish}
                  </Button>
                </div>
              )}
            </div>
